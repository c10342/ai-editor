...
export function renderTableButton(
  ...
  const items = [
    { label: '插入表格 (3×3)', action: () => insertTable(editorManager) },
  ...
  items.forEach(({ label, icon, action }) => {
    ...
    if (icon) {
      item.appendChild(getIconElement(icon));
    }
...
function insertTable(editorManager: any) {
